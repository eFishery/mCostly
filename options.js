...
const setValue = (node, value) => ({
  ...
  [node.type === 'checkbox']: () => { node.checked = value }
}).true()
...
const restoreOptions = () => {
  const idElements = Object.entries(id).reduce((acc, [key, id]) => ({
    ...acc,
    [key]: document.getElementById(id)
  }), {})

  const setValues = items => Object.entries(idElements).forEach(
    ([key, node]) => setValue(node, items[key] || defaultOptions[key])
  )
